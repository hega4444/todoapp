...
export default function TodoItem({ todo, list, onToggle, onEdit, onDelete }: TodoItemProps) {
    ...
    >
      ...
      >
        <div 
          ...
          style={{
            borderColor: todo.completed ? 'var(--status-success)' : 'var(--border-secondary)',
            backgroundColor: todo.completed ? 'var(--status-success)' : 'transparent'
          }}
          onMouseEnter={(e) => {
            if (!todo.completed) {
              e.currentTarget.style.borderColor = 'var(--interactive-secondary)';
            }
          }}
          onMouseLeave={(e) => {
            if (!todo.completed) {
      ...
      <button
        ...
        className={`absolute right-4 top-1/2 transform -translate-y-1/2 w-8 h-8 flex items-center justify-center rounded-full transition-all duration-200 ${
          isHovered && !isEditing ? 'opacity-100 visible' : 'opacity-0 invisible'
        }`}
